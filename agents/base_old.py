...
from utils import run_once, format_history, read_prompts
...
class BasePsychologicalAgent(ToolAgent):
    """
    Base class for all psychological agents in the recommendation system.
    Inherits from ToolAgent to support tool usage (e.g., knowledge retrieval, popularity lookup).
    """
    ...
    def __init__(
        ...
        """
        Initialize the base recommender agent.
    ...
    def rank_candidates(
        ...
        """
        Rank the candidate items based on agent's perspective.

        Args:
            candidate_set: Dictionary containing candidate_list and other metadata
            **kwargs: Additional arguments specific to each agent type

        Returns:
            Dictionary containing:
                - ranked_list: List of item_ids in ranked order
                - reasoning: Explanation for the ranking decision
        """
        pass
